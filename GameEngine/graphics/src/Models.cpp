...
glm::vec3 Model::getMaxModelScaling(void){
	return msizeMax;
}
	...
	void Model::loadModel(std::string const path)
	{
		...
		// set the max model size as a reference to scale engine (walls are the biggest so walls are the set scale its bad code shhhh it works)

		for (auto i : meshes)
		{
			for (auto v : i.vertices)
			{
				if (v.Position.x > msizeMax.x){
					msizeMax.x = v.Position.x;
				}
				else if (v.Position.y > msizeMax.y){
					msizeMax.y = v.Position.y;
				}
				else if (v.Position.z > msizeMax.z){
					msizeMax.z = v.Position.z;
				}
			}
		}

		// mSize = msizeMax - sizeMin;
	...
	void	Model::DrawAndSet(const Shader & shader, const std::string & name) {
		shader.setMat4(name, _Matrix);
		Draw(shader);
	}
	...
	void    Model::Rotate(const float& degrees) {
		_Matrix = glm::rotate(_Matrix, glm::radians(degrees), glm::vec3(0, 1, 0));
	}
